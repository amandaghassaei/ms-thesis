...
(if $angle$ == 0 then the axis does not matter, we can pick something arbitrary)
...
To calculate angular acceleration, we divide by moment of inertia.  For a 3D rectangular cell with sides $l_x$, $l_y$, $l_z$ and mass $m$, the tensor moment of inertia is given by:

\[ I = \dfrac{m}{12} \left[ \begin{array}{ccc}
l_y^2+l_z^2 & 0 & 0\\
0 & l_x^2+l_z^2 & 0\\
0 & 0 & l_x^2+l_y^2
 \end{array} \right] \]
...
Though I am not currently modeling this effect in simulation, it could be added once other bugs in the code have been resolved.
