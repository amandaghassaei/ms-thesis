...
In the simplest case without rotation, the force applied to cell 1 by cell 2 ($f_{2 \rightarrow 1}$) is given by:
...
If we wish to consider the orientations of the cells, denoted by the unit quaternions $q1$ and $q2$, we will need to adjust equation \ref{eq:translationalForce}. \\
  ...
  by treating $v$ as a 4-space vector [x, y, z, w] with w=0, using the Hamilton product:
...
a_wb_w - a_xb_x - a_yb_y - a_zb_z
 \end{array} \right] \] 
   ...
   with $q^*$ denoting the conjugate of $q$:
...
By performing a spherical linear interpolation (slerp) halfway between $q1$ and $q2$, we can calculate the average orientation of the two cells:
...
Using $q_{avg}$, we can rotate the stiffness and damping vectors from equation \ref{eq:translationalForce} into this average reference frame, and substitute the rotated vectors back into equation \ref{eq:translationalForce}:
 ...
 \[ \vec{f_{2\rightarrow1}} = \vec{k_{rot}} \circ (\vec{p_2} - \vec{p_1}) + \vec{d_{rot}} \circ (\vec{v_2} - \vec{v_1})\]
 
 rotate nom dist and apply to translation
