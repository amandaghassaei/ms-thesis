...
where $E$ is the elastic modulus (also called "Young's modulus") and G is the shear modulus of a given material.  As long as the forces acting on and within a solid are sufficiently low, we can assume that these linear elastic deformation models apply.\\
...
A material is isotropic if it responds the same to an external force no matter its orientation; materials comprised of oriented fibers or other internal structure typical display anisotropic behaviors.  Typically, FEA uses Equations \ref{eq:stressstrainaxial} and  \ref{eq:stressstrainshear}, which describe bulk material properties of isotropic materials.  $E$ and $G$ do not give enough information to describe the parts we intend to model.\\

In this analysis, we're interested in modeling the behavior of mutimaterial assemblies of isotropic and anisotropic building blocks, the \textit{functional primitives} from Chapter \ref{chapter:HierarchicalDesign}. \\
...
This thesis is concerned with the numerical solution of the position, orientation, and deformation of an elastic solid made from many elements of identical geometry and various material types.  Evaluating this system subject to its initial state and boundary conditions (applied forces, fixed regions, etc) gives the state of the assembly over time.\\
